...
app.get('/OICMP_ALL/:teamName', async(req, res, next) => {
        ...
        for (let index = 0; index < teamIps.length; index++) {
            ...
            var result = await session.pingHost(hostIn, function(error, hostIn, sent, rcvd) {
                ...
                if (error) {
                    var output = hostIn + ": " + error.toString()
                    Team.findOneAndUpdate({ name: name }, {
                            $push: {
                                [db_index]: { timeStamp: epochTime, status: false, error: error.toString() }
                            }
                        },
                        function(err, suc) {
                            if (err) {
                    ...
                    Team.findOneAndUpdate({ name: name }, {
                            $push: {
                                [db_index]: { timeStamp: epochTime, status: true, speed: ms }
                            }
                        },
                        function(err, suc) {
                            if (err) {
        ...
        for (let index = 0; index < teamIps.length; index++) {
            ...
            ssh.connect({
                ...
                ssh.execCommand(commandIn, { cwd: '' }).then(function(result) {
                    ...
                    Team.findOneAndUpdate({ name: name }, {
                            $push: {
                                [db_index]: { timeStamp: epochTime, status: true }
                            }
                        },
                        function(err, suc) {
                            if (err) {
    ...
    for (let index = 0; index < teamIps.length; index++) {
        ...
        dns.lookup(hostIn, function onLookup(err, addresses, family) {
            if (err) {
                ...
                Team.findOneAndUpdate({ name: name }, {
                        $push: {
                            [db_index]: { timeStamp: epochTime, status: false, error: err.toString() }
                        }
                    },
                    function(err, suc) {
                        if (err) {
    ...
    for (let index = 0; index < teamIps.length; index++) {
        ...
        con.connect(function(err) {
            if (err) {
                ...
                Team.findOneAndUpdate({ name: name }, {
                        $push: {
                            [db_index]: { timeStamp: epochTime, status: false, error: err }
                        }
                    },
                    function(err, suc) {
                        if (err) {
    ...
    for (let index = 0; index < teamIps.length; index++) {
        ...
        ad.authenticate(username, password, function(err, auth) {
            ...
            if (auth) {
                ...
                Team.findOneAndUpdate({ name: name }, {
                        $push: {
                            [db_index]: { timeStamp: epochTime, status: false, error: "auth failure" }
                        }
                    },
                    function(err, suc) {
                        if (err) {
